...
class BuilderSyncEventFromExternalData implements Core\Builders\Builder
{
	...
	public function prepareEvent(): Core\Event\Event
	{
		...
		if ($this->item['start'])
		{
			...
			elseif ($this->item['start']['dateTime'])
			{
				$event->setIsFullDay(false);
				$timeZone = Util::prepareTimezone($this->item['start']['timeZone']);
		...
		if (!empty($this->item['colorId']))
		{
			$event->setColor($this->item['colorId']);
		}
	...
	private function prepareRecurringRule(): ?RecurringEventRules
	{
		...
		$rrule = [];
		$parts = explode(':', $this->item['recurrence'][0]);
		if (empty($parts[1]))
		{
			return null;
		}
		...
		if ($rrule['FREQ'] && in_array($rrule['FREQ'], RecurringEventRules::FREQUENCY, true))
		{
		...
		if ((int)$rrule['COUNT'])
		{
			$property->setCount($rrule['COUNT']);
		}
		if ((int)$rrule['INTERVAL'])
		{
			$property->setInterval($rrule['INTERVAL']);
		}
		if ($rrule['UNTIL'])
		{
			try
		...
		if ($rrule['BYDAY'] && $rrule['FREQ'] === RecurringEventRules::FREQUENCY_WEEKLY)
		{
	...
	{
		if ($this->item['originalStartTime'])
		{
			if (!empty($this->item['originalStartTime']['dateTime']))
			{
				return Core\Base\Date::createDateTimeFromFormat($this->item['originalStartTime']['dateTime'],
						\DateTimeInterface::ATOM
					);
			}
	...
	private function getReminders(Core\Base\Date $start): RemindCollection
	{
		...
		if ($this->item['reminders']['overrides'])
		{
		...
		if ($this->item['reminders']['useDefault'])
		{
			$collection->add((new Core\Event\Properties\Remind())
