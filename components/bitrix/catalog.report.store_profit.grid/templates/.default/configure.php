...
Extension::load(['ui.buttons', 'crm.config.catalog']);
?>

<div class="catalog-report-config">
	<div class="catalog-report-config-title"><?= Loc::getMessage('CATALOG_REPORT_CONFIG_TITLE') ?></div>
</div>
...
<script>
	BX.ready(() => {
		const button = new BX.UI.Button({
			...
			onclick: () => {
				BX.Crm.Config.Catalog.Slider.open('report', {
					events: {
						onClose: () => {
