...
// устаревша€ функци€, оставлена дл€ совместимости
...
function SortingEx($By, $Path = false, $sByVar="by", $sOrderVar="order", $Anchor="nav_start")
{
	...
	//≈сли путь не задан, то будем брать текущий со всеми переменными
	if($Path===false)
		$Path = $APPLICATION->GetCurUri();

	//≈сли нет переменных, то надо добавл€ть параметры через ?
