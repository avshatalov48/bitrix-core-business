/* eslint-disable */
(function (exports,main_core,catalog_entityCard,main_core_events,ui_dialogs_messagebox) {
	'use strict';
	  ...
	  babelHelpers.createClass(VariationCard, [{
	    ...
	    value: function onSectionLayout(event) {
	      ...
	      main_core_events.EventEmitter.subscribe('BX.UI.EntityEditorList:onItemSelect', function (event) {
	        ...
	        ui_dialogs_messagebox.MessageBox.alert(main_core.Loc.getMessage('CPVD_QUANTITY_TRACE_NOTICE'), main_core.Loc.getMessage('CPVD_QUANTITY_TRACE_NOTICE_TITLE'), function (messageBox) {
	          babelHelpers.classPrivateFieldSet(_this2, _isQuantityTraceNoticeShown, false);
	          messageBox.close();
	        }, main_core.Loc.getMessage('CPVD_QUANTITY_TRACE_ACCEPT'));
...
}((this.window = this.window || {}),BX,BX.Catalog.EntityCard,BX.Event,BX.UI.Dialogs));
