...
if (Loader::includeModule('tasks'))
{
	if (
		...
		|| Context::getCurrent()->getRequest()->get('mode') === 'dev'
	)
	{
	...
	else
	{
		...
		if (!CSocNetFeatures::IsActiveFeature(SONET_ENTITY_USER, $arResult["VARIABLES"]["user_id"], "tasks"))
		{
			echo Loc::getMessage('SU_T_TASKS_UNAVAILABLE', [
				'#A_BEGIN#' => '<a href="' . str_replace(["#user_id#", "#USER_ID#"], $arResult['VARIABLES']['user_id'],
						$arResult['PATH_TO_USER_FEATURES']) . '">',
				'#A_END#' => '</a>',
			]);
		}
		else
		{
			Task::setUserId($arResult["VARIABLES"]["user_id"]);
			...
			$componentParams = [
				'TAGS_SLIDER' => "/company/personal/user/{$userId}/tasks/tags/",
				'TAGS_SLIDER_PATH_TO_USER_TASKS_TASK' => $arResult['PATH_TO_USER_TASKS_TASK'],
