(function (exports) {
	...
	(function () {
	  if (window.BXMailMessageController) return;
	  ...
	  BXMailMessageController.init = function (options) {
	    ...
	    if ('edit' != this.options.type) {
	      if (this.options.pageSize < 1 || this.options.pageSize > 100) this.options.pageSize = 5;
	      this.__log = {
	        'a': 0,
	        'b': 0
	      };
	      ...
	      for (var i in items) {
	        ...
	        if (typeof this.__log[log] != 'undefined') this.__log[log]++;
	        BX.bind(items[i], 'click', this.handleLogItemClick.bind(this, items[i].getAttribute('data-id')));
	      }

	      BX.Event.EventEmitter.subscribe('BXMailMessageActions:CRM_ACTIVITY', function (event) {
	        BX.show(BX.findChildByClassName(BX('mail-msg-view-details-' + event.getData().messageId), 'js-msg-view-control-skip', true));
	      });
	      BX.Event.EventEmitter.subscribe('BXMailMessageActions:CRM_EXCLUDE', function (event) {
	        BX.hide(BX.findChildByClassName(BX('mail-msg-view-details-' + event.getData().messageId), 'js-msg-view-control-skip', true));
	      });
	    }
	  ...
	  BXMailMessageController.initScrollable = function () {
	    if (!this.__scrollable) {
	      if (document.scrollingElement) this.__scrollable = document.scrollingElement;
	    }

	    if (!this.__scrollable) {
	      if (document.documentElement.scrollTop > 0 || document.documentElement.scrollLeft > 0) this.__scrollable = document.documentElement;else if (document.body.scrollTop > 0 || document.body.scrollLeft > 0) this.__scrollable = document.body;
	    }
	  ...
	  BXMailMessageController.scrollTo = function (node1, node2) {
	    ...
	    if (pos1.top < pos0.top) {
	      ...
	      this.scrollWrapper(Math.min(this.__scrollable.scrollTop - (pos0.top - pos1.top), this.__scrollable.scrollTop + (pos2.bottom - pos0.bottom)));
	    }
	  };

	  BXMailMessageController.handleLogClick = function (log, event) {
	    BX.PreventDefault(event);
	    var button = BX.findChildByClassName(BX('mail-msg-view-details-' + this.options.messageId).parentNode, 'mail-msg-view-log-more-' + log, true);
	  ...
	  BXMailMessageController.loadLog = function (log, button) {
	    ...
	    if (this['__loadingLog' + log]) return;
	    ...
	    BX.ajax({
	      method: 'POST',
	      url: BX.util.add_url_param(this.options.ajaxUrl, {
	        'action': 'log'
	      }),
	      data: data,
	      dataType: 'json',
	      onsuccess: function onsuccess(json) {
	        ...
	        if (json.status == 'success') {
	          ...
	          var marker = log == 'a' ? BX.findNextSibling(separator, {
	            'tag': 'div'
	          }) : separator;
	          ...
	          if (json.data.count < ctrl.options.pageSize) separator.style.display = 'none';
	  ...
	  BXMailMessageController.handleLogItemClick = function (messageId, event) {
	    ...
	    if (event.target && event.target.tagName && event.target.tagName.toUpperCase() == 'A') return;

	    if (window.getSelection) {
	      if (window.getSelection().toString().trim() != '') return;
	    } else if (document.selection) {
	      if (document.selection.createRange().htmlText.trim() != '') return;
	    }
	    ...
	    if (opened) {
	      ...
	      if (details.getAttribute('data-empty')) {
	        ...
	        BX.ajax({
	          ...
	          onsuccess: function onsuccess(json) {
	            if (json.status != 'success') {
	              json.errors = json.errors.map(function (item) {
	                return item.message;
	              });
	              details.innerHTML = '<div class="mail-msg-view-log-item-error mail-msg-view-border-bottom">' + json.errors.join('<br>') + '</div>';
	              return;
	            }
	            ...
	            setTimeout(function () {
	              ...
	              if (details.offsetHeight > 0) logItem.style.display = 'none';
	  ...
	  BXMailMessageController.close = function (destroy) {
	    ...
	    if (slider) {
	      ...
	      window.location.href = BX.util.add_url_param(this.options.pathList, {
	        'strict': 'N'
	      });
	    }
	  };

	  var BXMailMessage = function BXMailMessage(options) {
	    var self = this;
	    ...
	    if (this.htmlForm.__inited) return;

	    if ('edit' != this.ctrl.options.type) {
	      this.__wrapper = BX('mail-msg-view-details-' + this.ctrl.options.messageId);
	      if (this.options.messageId != this.ctrl.options.messageId) this.__wrapper = BX.findChildByClassName(this.__wrapper.parentNode, 'mail-msg-view-details-' + this.options.messageId, false);
	      BX.addCustomEvent('MailMessage:replyButtonClick', function (source) {
	        if (source !== self) self.hideReplyForm();
	      });
	      top.BX.SidePanel.Instance.postMessage(window, 'mail-message-view', {
	        id: this.options.messageId
	      });
	      var emailContainerId = 'mail_msg_' + this.options.messageId + '_body'; // target links

	      var emailLinks = typeof document.querySelectorAll != 'undefined' ? document.querySelectorAll('#' + emailContainerId + ' a') : BX.findChildren(BX(emailContainerId), {
	        tag: 'a'
	      }, true);

	      for (var i in emailLinks) {
	        if (!emailLinks.hasOwnProperty(i)) continue;
	        if (emailLinks[i] && emailLinks[i].setAttribute) emailLinks[i].setAttribute('target', '_blank');
	      } // unfold quotes


	      var quotesList = typeof document.querySelectorAll != 'undefined' ? document.querySelectorAll('#' + emailContainerId + ' blockquote') : BX.findChildren(BX(emailContainerId), {
	        tag: 'blockquote'
	      }, true);

	      for (var i in quotesList) {
	        if (!quotesList.hasOwnProperty(i)) continue;
	      ...
	      BX.bind(skipLink, 'click', function (event) {
	        BXMailMessageActions && BXMailMessageActions.createAction(event, {
	    ...
	    BX.addCustomEvent("onPullEvent-mail", BX.proxy(function (command, params) {
	      ...
	      if (items && items.length > 0) {
	        for (var i in items) {
	          BX.adjust(items[i], {
	            text: BX.message('MAIL_MESSAGE_READ_CONFIRMED_SHORT')
	          });
	        }
	      }
	    }, this));
	  ...
	  BXMailMessage.handleFormSubmit = function (form, event) {
	    ...
	    for (var i = 0; i < fields.length; i++) {
	      if ('data[to][]' == fields[i].name && fields[i].value.length > 0) emptyRcpt = false;
	    }
	    ...
	    for (var i in form.postForm.controllers) {
	      if (!form.postForm.controllers.hasOwnProperty(i)) continue;
	      if (form.postForm.controllers[i].storage != 'disk') continue;
	      ...
	      if (BX.message('MAIL_MESSAGE_MAX_SIZE') > 0) {
	        try {
	          ...
	          totalSize = Object.keys(items).reduce(function (sum, k) {
	  ...
	  BXMailMessage.prototype.showReplyForm = function (min) {
	    var mailForm = BXMainMailForm.getForm(this.options.formId);
	    var replyButton = BX.findChildByClassName(this.__wrapper, 'js-msg-view-reply-panel', true);
	    if (this.htmlForm.parentNode === this.__dummyNode) this.htmlForm.__wrapper.appendChild(this.htmlForm);
	    mailForm.init();
	  ...
	  BXMailMessage.prototype.markAsSpam = function (btn, uid) {
	    ...
	    }).then(this.onMessageActionSuccess.bind(this, btn), function (response) {
	  ...
	  BXMailMessage.prototype.onMessageActionError = function (response) {
	    alert(response.errors[0].message); // todo show errors
	  };

	  BXMailMessage.prototype.onMessageActionSuccess = function (btn) {
	    top.BX.SidePanel.Instance.postMessage(window, 'mail-message-reload-grid', {});
	    this.ctrl.close(true);
	  };
	  ...
	  BXMailMailbox.sync = function (stepper, gridId, onlySyncCurrent) {
	    ...
	    var pr = BX.ajax.runComponentAction('bitrix:mail.client', 'syncMailbox', {
	      mode: 'ajax',
	      data: {
	        id: self.mailbox.ID,
	    ...
	    pr.then(function (json) {
	      ...
	      BXMailMailbox.syncProgress(stepper, gridId, {
	        'complete': -1,
	        'status': -1,
	        'errors': json.errors
	      });
	    ...
	    if (complete < 0 && status < 0) {
	      ...
	      if (stepperError) {
	        ...
	        if (details.length > 0 && errors.length > 0) {
	          stepperError.appendChild(BX.UI.Hint.createNode(details.join(': ')));
	        }
	...
	(function () {
	  ...
	  top.BX.SidePanel.Instance.bindAnchors({
	    rules: [{
	      condition: ['^' + siteDir + 'mail/config/(new|edit)'],
	      options: {
	        width: 760,
	        cacheable: false
	      }
	    }, {
	      condition: ['^' + siteDir + 'mail/(blacklist|signature|config|message)'],
	      options: {
	        width: 1080
	      }
	    }]
