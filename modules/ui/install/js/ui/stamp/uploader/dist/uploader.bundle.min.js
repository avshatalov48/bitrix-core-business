this.BX=this.BX||{};this.BX.UI=this.BX.UI||{};(function(e,t,s,i,o,a,r,n,c){"use strict";let l=e=>e,h,u;class d{constructor(e){this.cache=new r.Cache.MemoryCache;this.setOptions(e)}setOptions(e){this.cache.set("options",{...e})}getOptions(){return this.cache.get("options",{})}setValue(e){if(r.Type.isString(e)||r.Type.isNumber(e)){this.getValueLayout().textContent=e}}getValueLayout(){return this.cache.remember("valueLayout",(()=>r.Tag.render(h||(h=l`
				<div class="ui-stamp-uploader-header-text-value">
					<span title="${0}">${0}</span>
				</div>
			`),r.Text.encode(this.getOptions().contact.label),r.Text.encode(this.getOptions().contact.label))))}getChangeContactButton(){return this.cache.remember("changeContactButton",(()=>new c.Button({text:r.Loc.getMessage("UI_STAMP_UPLOADER_HEADER_CHANGE_CONTACT_BUTTON_LABEL"),size:c.Button.Size.EXTRA_SMALL,color:c.Button.Color.LIGHT_BORDER,round:true})))}getLayout(){return this.cache.remember("headerLayout",(()=>r.Tag.render(u||(u=l`
			...
			`),this.getApplyButton().render(),this.getCancelButton().render())))}showCropAction(){r.Dom.show(this.getCropActionsLayout());r.Dom.hide(this.getCropButton())}hideCropActions(){r.Dom.hide(this.getCropActionsLayout());r.Dom.show(this.getCropButton())}getLayout(){return this.cache.remember("layout",(()=>r.Tag.render(A||(A=b`
				<div class="ui-stamp-uploader-action-panel">
					${0}
					${0}
				</div>
			`),this.getCropActionsLayout(),this.getCropButton())))}disable(){r.Dom.addClass(this.getLayout(),"ui-stamp-uploader-action-panel-disabled")}enable(){r.Dom.removeClass(this.getLayout(),"ui-stamp-uploader-action-panel-disabled")}}let E=e=>e,D,_,T,y;class I{constructor(){this.cache=new r.Cache.MemoryCache}static formatSize(e){if(e===0){return`0 ${r.Loc.getMessage("UI_STAMP_UPLOADER_UPLOAD_STATUS_SIZE_B")}`}const t=[r.Loc.getMessage("UI_STAMP_UPLOADER_UPLOAD_STATUS_SIZE_B"),r.Loc.getMessage("UI_STAMP_UPLOADER_UPLOAD_STATUS_SIZE_KB"),r.Loc.getMessage("UI_STAMP_UPLOADER_UPLOAD_STATUS_SIZE_MB")];const s=Math.floor(Math.log(e)/Math.log(1024));return{number:parseFloat((e/Math.pow(1024,s)).toFixed(2)),text:t[s]}}getUploadStatusLayout(){return this.cache.remember("statusLayout",(()=>{const e=r.Tag.render(D||(D=E`
				<div class="ui-stamp-uploader-upload-status-loader"></div>
			`));const t=new o.Loader({target:e,mode:"inline",size:45});void t.show();return r.Tag.render(_||(_=E`
