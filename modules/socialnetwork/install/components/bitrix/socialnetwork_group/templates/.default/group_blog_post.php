...
if(strlen($arResult["VARIABLES"]["post_id"]) > 0)
{
	...
	if(strlen($arParams["PATH_TO_USER_BLOG_POST"]) <= 0)	
		$arParams["PATH_TO_USER_BLOG_POST"] = "/company/personal/user/#user_id#/blog/#post_id#/";

	$dbPost = CBlogPost::GetList(array(), $arFilter, false, false, array("ID", "AUTHOR_ID"));
	if($arPost = $dbPost->Fetch())
	{
		LocalRedirect(CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_USER_BLOG_POST"], array("post_id"=>$arPost["ID"], "user_id" => $arPost["AUTHOR_ID"])));
		die();
	}
}
?>