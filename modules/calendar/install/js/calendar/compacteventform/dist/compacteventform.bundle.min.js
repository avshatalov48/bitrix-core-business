this.BX=this.BX||{};(function(e,t,i,s,n,o,r,a,l){"use strict";let c=e=>e,d,h,u,p,f,g,E,m,y,D,C,M,v,S,I,O,T,w,b,L,_,B,A,N,k,P,V,R,F,U,x,W,z,$;class X extends i.EventEmitter{constructor(e={}){super();this.STATE={READY:1,REQUEST:2,ERROR:3};this.zIndex=1200;this.Z_INDEX_OFFSET=-1e3;this.userSettings="";this.DOM={};this.displayed=false;this.sections=[];this.sectionIndex={};this.trackingUsersList=[];this.checkDataBeforeCloseMode=true;this.CHECK_CHANGES_DELAY=500;this.RELOAD_DATA_DELAY=500;this.excludedUsers=[];this.setEventNamespace("BX.Calendar.CompactEventForm");this.userId=e.userId||s.Util.getCurrentUserId();this.type=e.type||"user";this.isLocationCalendar=e.isLocationCalendar||false;this.calendarContext=e.calendarContext||null;this.ownerId=e.ownerId||this.userId;this.BX=s.Util.getBX();this.checkForChangesDebounce=t.Runtime.debounce(this.checkForChanges,this.CHECK_CHANGES_DELAY,this);this.reloadEntryDataDebounce=t.Runtime.debounce(this.reloadEntryData,this.RELOAD_DATA_DELAY,this);this.checkOutsideClickClose=this.checkOutsideClickClose.bind(this);this.outsideMouseDownClose=this.outsideMouseDownClose.bind(this);this.keyHandler=this.handleKeyPress.bind(this);this.lastUsedSaveOptions={}}show(e=X.EDIT_MODE,i={}){this.setParams(i);this.setMode(e);this.state=this.STATE.READY;this.popupId="compact-event-form-"+Math.round(Math.random()*1e5);if(this.popup){this.popup.destroy()}this.popup=this.getPopup(i);t.Dom.addClass(this.popup.titleBar,"calendar-add-popup-titlebar");t.Dom.removeClass(this.popup.popupContainer,"popup-window-with-titlebar");t.Dom.removeClass(this.popup.closeIcon,"popup-window-titlebar-close-icon");t.Event.bind(document,"mousedown",this.outsideMouseDownClose);t.Event.bind(document,"mouseup",this.checkOutsideClickClose);t.Event.bind(document,"keydown",this.keyHandler);t.Event.bind(this.popup.popupContainer,"transitionend",(()=>{t.Dom.removeClass(this.popup.popupContainer,"calendar-simple-view-popup-show")}));if(this.getMode()===X.EDIT_MODE){r.EntryManager.doDelayedActions()}this.prepareData().then((()=>{if(this.isLocationMode()){this.setFormValuesLocation()}else{this.setFormValues()}this.popup.show();if(this.userPlannerSelector&&(this.isLocationCalendar||this.userPlannerSelector.attendeesEntityList.length>1&&this.getMode()!==X.VIEW_MODE)){this.userPlannerSelector.showPlanner()}if(this.isTitleOverflowing()){this.DOM.titleInput.title=this.entry.name}this.checkDataBeforeCloseMode=true;if(this.canDo("edit")&&this.DOM.titleInput&&e===X.EDIT_MODE){this.DOM.titleInput.focus();this.DOM.titleInput.select()}this.displayed=true;if(this.getMode()===X.VIEW_MODE){s.Util.sendAnalyticLabel({calendarAction:"view_event",formType:"compact"});this.popup.getButtons()[0].button.focus()}if(this.getMode()===X.EDIT_MODE&&!this.userPlannerSelector.isPlannerDisplayed()){this.userPlannerSelector.checkBusyTime()}}))}getPopup(e){return new n.Popup(this.popupId,e.bindNode,{zIndex:this.zIndex+this.Z_INDEX_OFFSET,closeByEsc:true,offsetTop:0,offsetLeft:0,closeIcon:true,titleBar:true,draggable:true,resizable:false,lightShadow:true,className:"calendar-simple-view-popup calendar-simple-view-popup-show",cacheable:false,content:this.isLocationMode()?this.getPopupContentLocation():this.getPopupContentCalendar(),buttons:this.getButtons(),events:{onPopupClose:this.close.bind(this)}})}isShown(){return this.displayed}close(e=true,i=false){if(!e&&!this.checkTopSlider()){if(this.popup){this.popup.destroyed=true;setTimeout((()=>{this.popup.destroyed=false}),0)}return}if(this.getMode()===X.EDIT_MODE&&this.formDataChanged()&&this.checkDataBeforeCloseMode&&!i){this.showConfirmClosePopup();if(this.popup){this.popup.destroyed=true;setTimeout((()=>{this.popup.destroyed=false}),0)}return}this.displayed=false;this.emit("onClose");t.Event.unbind(document,"mousedown",this.outsideMouseDownClose);t.Event.unbind(document,"mouseup",this.checkOutsideClickClose);t.Event.unbind(document,"keydown",this.keyHandler);if(this.userPlannerSelector){this.userPlannerSelector.destroy()}if(this.popup){this.popup.destroy()}if(o.Location){o.Location.setCurrentCapacity(0)}s.Util.clearPlannerWatches();s.Util.closeAllPopups()}getPopupContentCalendar(){this.DOM.wrap=t.Tag.render(d||(d=c`<div class="calendar-add-popup-wrap">
			...
			</div>`),this.getEntryCounter(),this.getTitleControl(),this.getTitleFade(),this.getColorControl()),this.getSectionControl("textselect"),this.getDateTimeControl(),this.getUserPlannerSelector(),this.getTypeInfoControl(),this.getLocationControl(),this.DOM.remindersOuterWrap=t.Tag.render(u||(u=c`
				<div class="calendar-field-block">
		...
		`),t.Loc.getMessage("EC_ENTRY_NAME"));this.bindFade();t.Event.bind(this.DOM.titleInput,"keyup",this.checkForChangesDebounce);t.Event.bind(this.DOM.titleInput,"change",this.checkForChangesDebounce);t.Event.bind(this.DOM.titleInput,"keyup",this.updateEventNameInputTitle.bind(this));t.Event.bind(this.DOM.titleInput,"change",this.updateEventNameInputTitle.bind(this));return this.DOM.titleInput}bindFade(){let e=false;t.Event.bind(this.DOM.titleInput,"focusout",(()=>{if(this.DOM.titleInput.scrollWidth>this.DOM.titleInput.offsetWidth){this.getTitleFade().classList.add("--show")}else{this.getTitleFade().classList.remove("--show")}e=false}));t.Event.bind(this.DOM.titleInput,"focus",(()=>{this.getTitleFade().classList.remove("--show");e=true}));t.Event.bind(this.DOM.titleInput,"scroll",(()=>{if(this.DOM.titleInput.scrollWidth>this.DOM.titleInput.offsetWidth&&Math.ceil(this.DOM.titleInput.offsetWidth+this.DOM.titleInput.scrollLeft)<this.DOM.titleInput.scrollWidth&&!e){this.getTitleFade().classList.add("--show")}else{this.getTitleFade().classList.remove("--show")}}))}getTitleFade(){if(!this.DOM.titleFade){this.DOM.titleFade=t.Tag.render(y||(y=c`
			...
			<input class="calendar-field calendar-field-string --text-overflow-none"
				...
				readonly
			/>
		`),t.Loc.getMessage("EC_ENTRY_NAME"));this.bindFade();return this.DOM.titleInput}getHostControl(){const e=this.entry.data.CREATED_BY;const i=X.USER_URL.replace("#USER_ID#",e);const s=this.BX.Calendar.EntryManager.userIndex[e]?this.BX.Calendar.EntryManager.userIndex[e].AVATAR:"";this.DOM.hostBar=t.Tag.render(C||(C=c`
		...
		`),t.Loc.getMessage("EC_HOST")+": ",i,this.renderAvatar(s),i,BX.util.htmlspecialchars(this.entry.name));return this.DOM.hostBar}renderAvatar(e){const i="calendar-field-location-host-img-value";if(this.isAvatar(e)){return t.Tag.render(M||(M=c`
				<img class="${0}" src="${0}" alt="">
			`),i,e)}return t.Tag.render(v||(v=c`
			<div class="ui-icon ui-icon-common-user ${0}"><i></i></div>
		`),i)}isAvatar(e){return t.Type.isStringFilled(e)&&e!=="/bitrix/images/1.gif"}getColorControl(){this.DOM.colorSelect=t.Tag.render(S||(S=c`<div class="calendar-field calendar-field-select calendar-field-tiny"></div>`));this.colorSelector=new o.ColorSelector({wrap:this.DOM.colorSelect,mode:"selector"});this.colorSelector.subscribe("onChange",(e=>{if(e instanceof i.BaseEvent){const t=e.getData().value;if(!this.isNewEntry()&&(this.canDo("edit")||this.entry.getCurrentStatus()!==false)){this.BX.ajax.runAction("calendar.api.calendarajax.updateColor",{data:{entryId:this.entry.id,userId:this.userId,color:t}});this.entry.data.COLOR=t;this.emit("doRefresh");this.emitOnChange()}}}));return this.DOM.colorSelect}getColorControlsLocationView(){this.DOM.colorSelect=t.Tag.render(I||(I=c`<div class="calendar-field calendar-field-select calendar-colorpicker-readonly calendar-field-tiny"></div>`));this.colorSelector=new o.ColorSelector({wrap:this.DOM.colorSelect,mode:"view"});return this.DOM.colorSelect}getSectionControl(e){this.DOM.sectionSelectWrap=t.Tag.render(O||(O=c`<div class="calendar-field-choice-calendar"></div>`));this.sectionSelector=new o.SectionSelector({outerWrap:this.DOM.sectionSelectWrap,defaultCalendarType:this.type,defaultOwnerId:this.ownerId,sectionList:this.sections,sectionGroupList:a.SectionManager.getSectionGroupList({type:this.type,ownerId:this.ownerId,userId:this.userId,trackingUsersList:this.trackingUsersList}),mode:e,zIndex:this.zIndex,getCurrentSection:()=>{const e=this.getCurrentSection();if(e){return{id:e.id,name:e.name,color:e.color}}return false},selectCallback:e=>{if(e){if(this.colorSelector){this.colorSelector.setValue(e.color)}this.sectionValue=e.id;this.checkForChangesDebounce();a.SectionManager.saveDefaultSectionId(this.sectionValue)}}});return this.DOM.sectionSelectWrap}getDateTimeControl(){this.DOM.dateTimeWrap=t.Tag.render(T||(T=c`<div class="calendar-field-container calendar-field-container-datetime"></div>`));this.dateTimeControl=new o.DateTimeControl(null,{showTimezone:false,outerWrap:this.DOM.dateTimeWrap,inlineEditMode:true});this.dateTimeControl.subscribe("onSetValue",(()=>{this.excludedUsers=[]}));this.dateTimeControl.subscribe("onChange",(e=>{if(e instanceof i.BaseEvent){let t=e.getData().value;if(this.remindersControl){this.remindersControl.setFullDayMode(t.fullDay);if(this.isNewEntry()&&!this.remindersControl.wasChangedByUser()){const e=r.EntryManager.getNewEntryReminders(t.fullDay?"fullDay":"withTime");this.remindersControl.setValue(e,false)}}if(this.userPlannerSelector){if(!this.userPlannerSelector.isPlannerDisplayed()){this.userPlannerSelector.showPlanner()}this.userPlannerSelector.setLocationValue(this.locationSelector.getTextValue());this.userPlannerSelector.setDateTime(t,true);this.userPlannerSelector.refreshPlannerStateDebounce()}if(this.locationSelector){this.locationSelector.checkLocationAccessibility({from:e.getData().value.from,to:e.getData().value.to,fullDay:e.getData().value.fullDay,currentEventId:this.entry.id})}this.checkForChangesDebounce()}}));return this.DOM.dateTimeWrap}getUserPlannerSelector(){var e;this.DOM.userPlannerSelectorOuterWrap=t.Tag.render(w||(w=c`<div>
			...
			${0}
		<div>`),this.DOM.userSelectorWrap=t.Tag.render(b||(b=c`
				<div class="calendar-field-block">
				...
				</div>`),t.Loc.getMessage("EC_ATTENDEES_MORE"),t.Loc.getMessage("EC_SEC_SLIDER_CHANGE")),this.DOM.informWrap=t.Tag.render(L||(L=c`
				<div class="calendar-field-container-inform">
					<span class="calendar-field-container-inform-text">${0}</span>
				</div>`),t.Loc.getMessage("EC_NOTIFY_OPTION")),this.DOM.plannerOuterWrap=t.Tag.render(_||(_=c`
