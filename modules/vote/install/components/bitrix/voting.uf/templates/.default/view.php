...
UI\Extension::load([ 'ui.buttons', 'ajax', 'popup' ]);
			...
			<div class="bx-vote-block">
				...
				foreach ($attach['QUESTIONS'] as $question)
				{
					...
					<li id="question<?= $question["ID"] ?>"<? if ($question["REQUIRED"] === "Y"): ?> class="bx-vote-question-required"<? endif; ?>>
						...
						{
							?>
							<div class="bx-vote-question-image"><img src="<?=$question["IMAGE"]["SRC"]?>"/></div><?
						}
						?>
...
<?
	...
	?><div class="bx-vote-buttons"><?
		?><button class="ui-btn ui-btn-lg ui-btn-link bx-vote-button-resubmit" data-bx-vote-button="showVoteForm"><?= Loc::getMessage('VOTE_RESUBMIT_BUTTON') ?></button><?
		?><button class="ui-btn ui-btn-lg ui-btn-primary" data-bx-vote-button="actVoting"><?= Loc::getMessage('VOTE_SUBMIT_BUTTON') ?></button><?
		if ($attach->canEdit($USER->GetID()))
		{
			?><span data-bx-vote-button="stopOrResume"><?
				?><button class="ui-btn ui-btn-lg ui-btn-link bx-vote-button-stop"><?= Loc::getMessage('VOTE_STOP_BUTTON') ?></button><?
				?><button class="ui-btn ui-btn-lg ui-btn-link bx-vote-button-resume"><?= Loc::getMessage('VOTE_RESUME_BUTTON')?></button><?
			?></span><?
			?><button class="ui-btn ui-btn-lg ui-btn-link" data-bx-vote-button="exportXls"><?= Loc::getMessage('VOTE_EXPORT_BUTTON') ?></button><?
		}
	?></div><?
	?>
