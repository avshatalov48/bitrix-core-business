	  ...
	  methods: {
	    ...
	    getFirstPage: function getFirstPage() {
	      ...
	      this.getRestClient().callMethod(im_const.RestMethod.imRecentList, queryParams).then(function (result) {
	        ...
	        _this.lastMessageDate = _this.getLastMessageDate(result.data().items); //if we got less items than page count = no more items

	        if (!result.data().hasMore) {
	          _this.hasMoreToLoad = false;
	        }

	        _this.$store.dispatch('recent/clearPlaceholders'); //set first chunk of real data in rest handler
	    ...
	    loadNextPage: function loadNextPage() {
	      ...
	      this.loadingMore = true; //get first placeholder which we need to update with new data

	      this.firstPlaceholderToUpdate = this.placeholderCount; //draw new placeholders and get next items from backend
	    ...
	    getNextPage: function getNextPage() {
	      ...
	      this.getRestClient().callMethod(im_const.RestMethod.imRecentList, queryParams).then(function (result) {
	        var items = result.data().items; //if we got nothing - clear placeholders
	        ...
	        _this3.updateModels(items).then(function () {
	          _this3.loadingMore = false; //if we got less items than page count - clear remaining placeholders
