<section class="landing-block g-pt-80 g-pb-80">
	<div class="container">
		<div class="row">
			<div class="col-md-6 col-lg-4 g-mb-30 g-mb-0--lg">
				<!-- Article -->
				<article class="landing-block-node-block js-animation fadeInUp text-center u-block-hover u-bg-overlay g-color-white g-bg-img-hero g-bg-black-opacity-0_3--after">
					<!-- Article Image -->
					<img class="landing-block-node-img1 w-100 u-block-hover__main--zoom-v1" src="https://cdn.bitrix24.site/bitrix/images/landing/business/800x867/img1.jpg"
						 alt="">
					<!-- End Article Image -->

					<!-- Article Content -->
					<div class="u-block-hover__additional u-bg-overlay__inner g-pos-abs g-flex-middle g-brd-around g-brd-2 g-brd-white-opacity-0_3 g-pa-15 g-ma-20">
						<div class="text-uppercase g-flex-middle-item">
							<h3 class="landing-block-node-subtitle1 landing-semantic-text-image-medium font-weight-bold g-font-size-18 g-color-white g-brd-bottom g-brd-2 g-brd-primary g-mb-20">
								Face massage</h3>
							<div class="landing-block-node-title1 landing-semantic-text-image-big text-uppercase g-line-height-1 font-weight-bold g-font-size-40 g-mb-30">
								<span style="font-weight: bold;">40% Off</span>
							</div>
							<div class="landing-block-node-button1-container">
								<a class="landing-block-node-button1 landing-semantic-link-image-medium btn btn-md text-uppercase u-btn-primary font-weight-bold g-font-size-11 g-brd-none rounded-0 g-py-10 g-px-25"
								   href="#">Contact us</a>
							</div>
						</div>
					</div>
					<!-- End Article Content -->
				</article>
				<!-- End Article -->
			</div>

			<div class="col-md-6 col-lg-4 g-mb-30 g-mb-0--lg">
				<!-- Article -->
				<article class="landing-block-node-block js-animation fadeInUp text-center u-block-hover u-bg-overlay g-color-white g-bg-img-hero g-bg-black-opacity-0_3--after">
					<!-- Article Image -->
					<img class="landing-block-node-img2 w-100 u-block-hover__main--zoom-v1" src="https://cdn.bitrix24.site/bitrix/images/landing/business/800x867/img2.jpg"
						 alt="">
					<!-- End Article Image -->

					<!-- Article Content -->
					<div class="u-block-hover__additional u-bg-overlay__inner g-pos-abs g-flex-middle g-brd-around g-brd-2 g-brd-white-opacity-0_3 g-pa-15 g-ma-20">
						<div class="text-uppercase g-flex-middle-item">
							<h3 class="landing-block-node-subtitle2 landing-semantic-text-image-medium font-weight-bold g-font-size-16 g-color-white g-mb-5">
								20% Discount</h3>
							<div class="landing-block-node-title2 landing-semantic-text-image-big text-uppercase g-line-height-1 font-weight-bold g-font-size-28 g-mb-10">
								<span style="font-weight: bold;">Products of spa</span>
							</div>
							<h3 class="landing-block-node-text2 landing-semantic-text-image-medium font-weight-bold g-font-size-16 g-color-white mb-0">
								When you buy over 100$</h3>
						</div>
					</div>
					<!-- End Article Content -->
				</article>
				<!-- End Article -->
			</div>

			<div class="col-lg-4">
				<!-- Article -->
				<article class="landing-block-node-bg-mini js-animation fadeInUp text-center u-block-hover g-color-white g-bg-primary g-mb-30">
					<div class="g-brd-around g-brd-2 g-brd-white-opacity-0_3 g-pa-30 g-ma-20">
						<div class="g-flex-middle-item">
							<h3 class="landing-block-node-title-mini landing-semantic-text-image-medium text-uppercase font-weight-bold g-font-size-18 g-color-white g-mb-10">
								Relaxing
								massage</h3>
							<div class="landing-block-node-text-mini landing-semantic-text-image-small g-font-size-12 g-color-white mb-0">
								<p>Morbi ex urna, porttitor vel consequat non</p>
							</div>
						</div>
					</div>
				</article>
				<!-- End Article -->

				<!-- Article -->
				<article class="landing-block-node-block js-animation fadeInUp text-center u-block-hover u-bg-overlay g-color-white g-bg-img-hero g-bg-black-opacity-0_3--after">
					<!-- Article Image -->
					<img class="landing-block-node-img-mini w-100 u-block-hover__main--zoom-v1" src="https://cdn.bitrix24.site/bitrix/images/landing/business/800x401/img1.jpg"
						 alt="">
					<!-- End Article Image -->

					<!-- Article Content -->
					<div class="u-block-hover__additional u-bg-overlay__inner g-pos-abs g-flex-middle g-brd-around g-brd-2 g-brd-white-opacity-0_3 g-pa-15 g-ma-20">
						<div class="g-flex-middle-item">
							<h3 class="landing-block-node-title-mini landing-semantic-text-image-medium text-uppercase font-weight-bold g-font-size-18 g-color-white g-mb-5">
								1+1=1</h3>
							<div class="landing-block-node-text-mini landing-semantic-text-image-small g-font-size-12 g-color-white mb-0">
								<p>Morbi ex urna, porttitor vel consequat non</p>
							</div>
						</div>
					</div>
					<!-- End Article Content -->
				</article>
				<!-- End Article -->
			</div>
		</div>
	</div>
</section>