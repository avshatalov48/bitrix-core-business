...
if (($_GET['showOldToCompare'] ?? 'N') === 'Y')
{
	...
	$channels = [];
	if ($dbRes = \Bitrix\Vote\Channel::getList([
		'select' => ['ID', 'TITLE'],
		'filter' => ($moduleRights < 'W' ? [
			'ACTIVE' => 'Y',
			'HIDDEN' => 'N',
	...
	$channelsToAdmin = array_keys($channels);
	if ($moduleRights < 'W')
	{
		$channelsToAdmin = [];
		$dbRes = Vote\Channel::getList([
			'select' => ['ID'],
			'filter' => [
				'ACTIVE' => 'Y',
				'HIDDEN' => 'N',
	...
	$arFilterFields = Array(
		...
		"find_counter_1",
		"find_counter_2");
	$arFilter = ($moduleRights < "W" ? array("@CHANNEL_ID" => array_keys($channels)) : array());
	 ...
	 ********************************************************************/
	// region Reset action
	if (intval($request->getQuery("reset_id")) > 0 && $moduleRights >= "W" && check_bitrix_sessid())
	{
	...
	while($res = $rsData->fetch())
	{
		...
		if (in_array($res["CHANNEL_ID"], $channelsToAdmin))
		{
			...
			$row->AddSelectField("CHANNEL_ID", $channels);
	...
	if ($moduleRights >= "W" || !empty($channelsToAdmin)):
	...
	<form name="find_form" method="GET" action="<?=$APPLICATION->GetCurPage()?>?">
		...
		<tr>
			...
			<td nowrap><?=SelectBoxFromArray("find_channel_id", array("reference" => array_values($channels), "reference_id" => array_keys($channels)), $arFilter["CHANNEL_ID"], GetMessage("VOTE_ALL"))?></td>
		</tr>
		<tr>
		...
		<tr>
			<td>ID:</td>
			<td><input type="text" name="find_id" size="47" value="<?=htmlspecialcharsbx($arFilter["ID"])?>"><?=InputType("checkbox", "find_id_exact_match", "Y", $request->getQuery("find_id_exact_match"), false, "", "title='".GetMessage("VOTE_EXACT_MATCH")."'")?>&nbsp;<?=ShowFilterLogicHelp()?></td>
		</tr>
		<tr>
