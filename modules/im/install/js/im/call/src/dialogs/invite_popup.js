...
export class InvitePopup
{
	...
	getRecent()
	{
		...
		{
			CallEngine.getRestClient().callMethod("im.recent.get", {
				"SKIP_OPENLINES": "Y",
				"SKIP_CHAT": "Y"
			}).then((response) =>
			{
	...
	search()
	{
		return new Promise((resolve) =>
		{
			...
			CallEngine.getRestClient().callMethod("im.search.user.list", {"FIND": this.searchPhrase}).then((response) =>
			{
				const answer = response.answer;
	...
	fetchMoreSearchResults()
	{
		return new Promise((resolve) =>
		{
			CallEngine.getRestClient().callMethod("im.search.user.list", {
