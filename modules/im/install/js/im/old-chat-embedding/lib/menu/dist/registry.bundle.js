...
(function (exports,main_popup,main_core_events,ui_dialogs_messagebox,im_oldChatEmbedding_const,main_core) {
	'use strict';
	...
	class PinManager {
	  ...
	  pinDialog(dialogId) {
	    ...
	    this.restClient.callMethod(im_oldChatEmbedding_const.RestMethod.imRecentPin, queryParams).catch(error => {
	...
	class UnreadManager {
	  ...
	  readDialog(dialogId) {
	    ...
	    if (dialog.counter > 0) {
	      ...
	      this.restClient.callMethod(im_oldChatEmbedding_const.RestMethod.imDialogRead, queryParams).catch(error => {
	    ...
	    this.restClient.callMethod(im_oldChatEmbedding_const.RestMethod.imRecentUnread, queryParams).catch(error => {
	      console.error('Im.RecentList: error reading chat', error);
	      this.store.dispatch('recent/unread', {
	        id: dialogId,
	        action: true
	...
	class MuteManager {
	  ...
	  muteDialog(dialogId) {
	    ...
	    this.restClient.callMethod(im_oldChatEmbedding_const.RestMethod.imChatMute, queryParams).catch(error => {
	...
	class RecentMenu extends BaseMenu {
	  ...
	  getSendMessageItem() {
	    return {
	      ...
	      onclick: function () {
	        const target = this.context.target === im_oldChatEmbedding_const.OpenTarget.current ? im_oldChatEmbedding_const.OpenTarget.current : im_oldChatEmbedding_const.OpenTarget.auto;
	  ...
	  getMuteItem() {
	    ...
	    const muteAllowed = this.store.getters['dialogues/getChatOption'](dialog.type, im_oldChatEmbedding_const.ChatOption.mute);
	    if (!muteAllowed) {
	      return null;
	    }
	  ...
	  getCallItem() {
	    ...
	    const isChat = dialog.type !== im_oldChatEmbedding_const.DialogType.user;
	    const callAllowed = this.store.getters['dialogues/getChatOption'](dialog.type, im_oldChatEmbedding_const.ChatOption.call);
	    ...
	    return {
	      ...
	      onclick: function () {
	        main_core_events.EventEmitter.emit(im_oldChatEmbedding_const.EventType.dialog.call, this.context);
	        this.menuInstance.close();
	      }.bind(this)
	    };
	  }
	  getHistoryItem() {
	    ...
	    return {
	      ...
	      onclick: function () {
	        const target = this.context.target === im_oldChatEmbedding_const.OpenTarget.current ? im_oldChatEmbedding_const.OpenTarget.current : im_oldChatEmbedding_const.OpenTarget.auto;
	        main_core_events.EventEmitter.emit(im_oldChatEmbedding_const.EventType.dialog.openHistory, {
	          ...this.context,
	          chat: this.store.getters['dialogues/get'](this.context.dialogId, true),
	          user: this.store.getters['users/get'](this.context.dialogId, true),
	          target
	  ...
	  getHideItem() {
	    ...
	    return {
	      ...
	      onclick: function () {
	        main_core_events.EventEmitter.emit(im_oldChatEmbedding_const.EventType.dialog.hide, {
	          ...this.context,
	          chat: this.store.getters['dialogues/get'](this.context.dialogId, true),
	          user: this.store.getters['users/get'](this.context.dialogId, true)
	        });
	  ...
	  getLeaveItem() {
	    ...
	    let optionToCheck = im_oldChatEmbedding_const.ChatOption.leave;
	    if (dialog.owner === this.getCurrentUserId()) {
	      optionToCheck = im_oldChatEmbedding_const.ChatOption.leaveOwner;
	    }
	    ...
	    return {
	      ...
	      onclick: function () {
	        main_core_events.EventEmitter.emit(im_oldChatEmbedding_const.EventType.dialog.leave, {
	          ...this.context,
	          chat: this.store.getters['dialogues/get'](this.context.dialogId, true),
	          user: this.store.getters['users/get'](this.context.dialogId, true)
	        });
...
}((this.BX.Messenger.Embedding.LibLegacy = this.BX.Messenger.Embedding.LibLegacy || {}),BX.Main,BX.Event,BX.UI.Dialogs,BX.Messenger.Embedding.Const,BX));
