			...
			if ($ob->Init() && $ob->ACTIVE === 'Y')
			{
				...
				)
				{
					...
					if (!file_exists($absPath))
					{
						...
						$fp = fopen($absTempPath, 'ab');
						$request = new \Bitrix\Main\Web\HttpClient([
							'redirect' => true,
						]);
						$request->setOutputStream($fp);
						if (is_resource($fp))
						{
							if ($arFile['FILE_SIZE'] > $maxPartSize)
							{
								$firstByte = $last_file_pos;
								$lastByte = min($arFile['FILE_SIZE'], $last_file_pos + $maxPartSize) - 1;
								$request->setHeader('Range', sprintf('bytes=%u-%u', $firstByte, $lastByte));
							}

							$result = $request->query(\Bitrix\Main\Web\Http\Method::GET, $ob->GetFileSRC($arFile));
							if ($result && ($request->getStatus() === 200 || $request->getStatus() === 206))
							{
								$request->getResult();
								fclose($fp);
								...
								if (
									...
									&& preg_match('/(\\d+)-(\\d+)\\/(\\d+)$/', $request->getHeaders()->get('Content-Range'), $match)
								)
								{
									$FILE_SIZE = $match[3];
								}
								elseif (
									$request->getHeaders()->has('Content-Length')
									&& preg_match('/^(\\d+)$/', $request->getHeaders()->get('Content-Length'), $match)
									&& $match[1] > $maxPartSize
								)
								{
					...
					if ($bNextFile)
					{
						...
						if ($bFileMoved)
						{
							...
							$filesToUpdate = [intval($arFile['ID'])];
					...
					CAdminMessage::ShowMessage([
						...
						'DETAILS' => GetMessage('CLO_STORAGE_LIST_DOWNLOAD_PROGRESS', [
		...
		case 'move':
			...
			if ($ob->ACTIVE === 'Y' && $ob->READ_ONLY === 'N')
			{
				...
				)
				{
					...
					if ($original)
					{
						$ob->Update(['LAST_FILE_ID' => $arFile['ID']]);
						$counter++;
						continue;
					}
				...
				elseif ($counter < $files_per_step && !$bWasMoved)
				{
					CAdminMessage::ShowMessage([
						...
						'DETAILS' => GetMessage('CLO_STORAGE_LIST_MOVE_PROGRESS', [
		...
		case 'estimate_duplicates':
			...
			if ($ob->ACTIVE === 'Y' && $ob->READ_ONLY === 'N' && $ob->Init())
			{
				...
				else
				{
					...
					{
						$lAdmin->AddUpdateError(GetMessage('CLO_STORAGE_LIST_UNKNOWN_ERROR', [
							'#CODE#' => 'D03',
						]), $ID);
					}
				}
			...
			if ($ob->ACTIVE === 'Y' && $ob->READ_ONLY === 'N' && $ob->Init())
			{
				...
				else
				{
					...
					$message = new CAdminMessage([
						...
						'DETAILS' => GetMessage('CLO_STORAGE_LIST_DUPLICATES_INFO', [
...
$rsData = CCloudStorageBucket::GetList(['SORT' => 'DESC', 'ID' => 'ASC']);
