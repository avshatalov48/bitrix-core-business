<section class="landing-block landing-block-node-bgimg u-bg-overlay g-bg-img-hero g-bg-attachment-fixed g-bg-black-opacity-0_6--after g-pt-100 g-pb-100"
		 style="background-image: url(https://cdn.bitrix24.site/bitrix/images/landing/business/1400x588/img1.jpg);">
	<div class="container u-bg-overlay__inner">
		<div class="container text-center g-width-780 g-mb-65">
			<div class="landing-block-node-header u-heading-v2-2--bottom g-brd-primary">
				<h2 class="landing-block-node-title landing-semantic-title-image-medium js-animation slideInDown text-uppercase u-heading-v2__title g-line-height-1 font-weight-bold g-font-size-40 g-color-white g-mb-0">
					Upcoming events</h2>
			</div>
		</div>

		<div class="row text-center text-md-left">
			<div class="landing-block-node-subtitle-container js-animation slideInLeft col-md-5 col-lg-5 offset-md-1">
				<h3 class="landing-block-node-subtitle landing-semantic-subtitle-image-medium text-uppercase font-weight-bold g-font-size-26 g-color-white g-mb-5">
					New album presetnation</h3>
				<div class="landing-block-node-text landing-semantic-text-image-medium g-font-size-13 g-color-white g-mb-60 g-mb-0--md">
					<p>
						8 Rue de Montpensier 75001, Paris, France, 18:00
					</p>
				</div>
			</div>

			<div class="landing-block-node-date-container js-animation slideInRight col-md-6 col-lg-6">
				<!-- Countdown -->
				<div class="u-countdown-v1 g-color-white">
					<div class="d-inline-block text-center g-bg-white-opacity-0_05 g-brd-around g-brd-white-opacity-0_3 g-pa-15-20-10 g-mx-15 g-mb-30">
						<div class="landing-block-node-date-value landing-semantic-subtitle-image-medium g-line-height-1 g-font-size-40 font-weight-bold g-mb-2">12</div>
						<div class="landing-block-node-date-text landing-semantic-text-image-medium text-uppercase text-center g-color-white-opacity-0_7 g-font-size-12">Day</div>
					</div>

					<div class="d-inline-block g-bg-white-opacity-0_05 g-brd-around g-brd-white-opacity-0_3 g-pa-15-20-10 g-mx-15 g-mb-30">
						<div class="landing-block-node-date-value landing-semantic-subtitle-image-medium g-line-height-1 g-font-size-40 font-weight-bold g-mb-2">02</div>
						<div class="landing-block-node-date-text landing-semantic-text-image-medium text-uppercase text-center g-color-white-opacity-0_7 g-font-size-12">Month</div>
					</div>

					<div class="d-inline-block g-bg-white-opacity-0_05 g-brd-around g-brd-white-opacity-0_3 g-pa-15-20-10 g-mx-15 g-mb-30">
						<div class="landing-block-node-date-value landing-semantic-subtitle-image-medium g-line-height-1 g-font-size-40 font-weight-bold g-mb-2">2021</div>
						<div class="landing-block-node-date-text landing-semantic-text-image-medium text-uppercase text-center g-color-white-opacity-0_7 g-font-size-12">Year</div>
					</div>
					
				</div>
				<!-- End Countdown -->
			</div>
		</div>
		
	</div>
</section>
 